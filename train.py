...
if __name__ == '__main__':
    ...
    for current_epoch in range(all_epoch):
        ...
        for idx, (train_x, train_label) in enumerate(train_loader):
            if(idx%100==99):
                print("第{}轮-进度{}/50000".format(current_epoch,(idx+1)*batch_size))
            ...
            loss.backward()
            sgd.step()
        print("第{}轮训练完成，开始测试".format(current_epoch))
        all_correct_num = 0
        all_sample_num = 0
        model.eval()
        total_epoch = []
        ...
        for idx, (test_x, test_label) in enumerate(test_loader):
            ...
            # print(current_correct_num.to('cpu').numpy()) 可以看看他长啥样
            all_correct_num += np.sum(current_correct_num.to('cpu').numpy(), axis=-1)
            all_sample_num += current_correct_num.shape[0]
        acc = all_correct_num / all_sample_num
        total_acc.append(current_epoch)
        total_acc.append(acc)
        print('第{}轮完成,准确率: {:.3f}'.format(current_epoch,acc), flush=True)
